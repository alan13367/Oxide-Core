...
- **`oxide_asset`**: Generic typed handles, revision-tracked handle-indexed asset storage with added/modified/removed change records and per-consumer change cursors, async loading/reload primitives, load status, typed path identity, labeled sub-asset identity, and dependency path metadata for hot reload/import invalidation. Renderer-specific caches and imported asset types should live in renderer or engine-facing crates.
