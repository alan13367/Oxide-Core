...
- **Async glTF Spawn Flow**: Queue loads via `request_gltf_scene_spawn(...)`; `gltf_scene_spawn_system` resolves handles, publishes imported meshes into `MeshCache` as labeled assets when source paths are known, and spawns hierarchy roots retrievable through `take_spawned_scene_roots(...)`.
