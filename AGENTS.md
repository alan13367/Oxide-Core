...
- **Change Revisions**: Use `World::change_tick()`, `component_revision`, `component_changed_since`, `component_added_revision`, `component_added_since`, `ComponentChanges<T>`, `removed_components_since`, `RemovedComponents<T>`, `prune_removed_components_through`, `resource_revision`, `resource_changed_since`, `Query<(Entity, &T)>::iter_added_since`, `Query<(Entity, &T)>::iter_changed_since`, `query_filtered::<&T, Changed<T>>()`, `query_filtered::<(Entity, &T), Added<T>>()`, and `ResourceCursor<T>` for lightweight cache invalidation in renderer, editor, tooling, and gameplay systems.
