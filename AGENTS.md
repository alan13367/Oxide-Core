...
- **Render Pass Ordering**: Plugins can register lightweight frame callbacks with `add_render_pass`, `add_render_pass_before`, `add_render_pass_after`, and render pass sets. Built-in anchors are `RENDER_PASS_SCENE`, `RENDER_PASS_GAME_TEXT`, `RENDER_PASS_APP_QUEUE`, and `RENDER_PASS_EGUI`. Use `RenderPassSchedule::ordered_pass_infos` / `pass_infos` for tooling snapshots, `enable_render_pass` / `disable_render_pass` for temporary custom-pass toggles, and `enable_render_pass_set` / `disable_render_pass_set` for plugin-owned pass groups.
- **Input Mapping**: `oxide_input` supports raw keyboard/mouse resources plus semantic `ActionBindings<T>`/`ActionInput<T>` synced by `sync_action_input_system::<T>`; bindings can be replaced, unbound, inspected for conflicts, and layered by mode with `InputContexts<T>` plus contextual action/axis bindings.
...
- **`oxide_engine`**: The core orchestration/facade layer. It defines the `App` trait, plugin APIs (`Plugin`, `DefaultPlugins`, `SceneAuthoringPlugins`) with registration metadata and duplicate guards, manages the main loop via `winit`, integrates `oxide_ecs` schedules/systems, handles input/events, provides an `AssetWatcher`, registers native scene/material asset loaders, unified renderer/native scene/material/glTF reload helpers, native scene material texture publication, material descriptor asset helpers for hot-reloading, lightweight transform tween animation, imported transform animation clip playback, weighted blending, and named animation state transitions, skeletal skin asset plumbing, post-transform skin joint matrix updates, and CPU-skinned runtime mesh uploads for the scene renderer, and owns engine-facing plugin wiring/re-exports for scene, UI, editor, camera, light, audio, and renderer systems.
- **`oxide_renderer`**: A low-level abstraction over `wgpu`. It handles device/queue initialization, swapchain management (Surface), and provides primitives for meshes with CPU vertex/index mirrors plus optional skinning attributes, CPU-side texture images, GPU textures, sampled `RenderTexture` color targets, fullscreen blit pipelines, descriptor-driven materials, shaders, and lightweight glTF import data including transform animation channels, skin bind data, and primitive joint/weight attributes.
...
- **`oxide_asset`**: Generic typed handles, revision-tracked handle-indexed asset storage with added/modified/removed change records and per-consumer change cursors, registered typed extension loaders, async loading/reload primitives, ready imported asset registration, load status, typed path identity, labeled sub-asset identity, and dependency path metadata for hot reload/import invalidation. Renderer-specific caches and imported asset components should live behind Oxide-owned scene/renderer/engine-facing APIs.
...
- **`oxide_scene`**: Scene descriptors, versioned `.oxscene` documents, validation/export diagnostics, structured scene/prefab spawn results, live-world scene/prefab export for editor save flows, authored `SceneInstanceId`, `SceneEntityPath`, and `Tags` plus indexed scoped query/unload helpers, scene-declared dependency metadata and material registration with base-color/albedo/normal/metallic/roughness texture authoring, scene-authored `RenderBounds`/`RenderCullDistance` culling metadata, gameplay `pick_scene` / `viewport_pick_ray` helpers, nested entity/prefab/sprite spawning with prefab instance overrides, renderable scene components, named `SceneMaterialLibrary` materials, camera/renderable `RenderLayers`, explicit `RenderBounds`/`RenderCullDistance` culling components, native RGBA/PNG sprites, terrain/world descriptors, transform hierarchy re-exports, scene gizmo line overlays, and the automatic `SceneRenderer` with swapchain and offscreen target queueing. Engine integration lives in `oxide_engine::scene::SceneRendererPlugin` and `PickingPlugin`, which expose automatic scene rendering plus cursor-derived picking state/events while keeping renderer and input plumbing behind Oxide APIs.
...
- **`examples/`**: Contains demonstration projects. The primary example is `hello_window`, which serves as a full-featured interactive 3D scene with FPS-style camera controls. Other examples include `minimal_game`, `material_filter_example`, `zombie_shooter`, `physics_example`, `unlit_example`, `sky_gradient_example`, and `sprite_ui_example`.
...
8. If `SceneRendererPlugin` is installed, the runner automatically prepares and queues visible `RenderMesh`, `Terrain`, `SpriteBillboard`, and scene gizmo lines at the `RENDER_PASS_SCENE` anchor, draws all active cameras by `CameraRenderView` ordering, filters renderables through each camera's `RenderLayers`, culls bounded renderables against each camera frustum plus optional `RenderCullDistance`, records `SceneRendererStats` for camera views, candidates, culls, submitted instances, and draw calls, publishes those counters to `Diagnostics` under `SCENE_*` labels, applies camera viewport/aspect metadata, and uses the first view clear color when present, then resizes the scene depth texture. If `GameUiPlugin` is installed, it also installs the native overlay text renderer and `GameFonts` registry used by `GameUi` text widgets. If `EguiPlugin` is installed, the runner owns egui input, frame execution, and rendering at `RENDER_PASS_EGUI`. Egui authoring panels are hidden by default and toggled with `F1`.
...
- **Render Passes**: Use `add_render_pass` for plugin-owned overlays or debug drawing that should run between built-in game text and `App::queue`. Use `add_render_pass_before` / `add_render_pass_after` for explicit ordering around built-in anchors, inspect resolved ordering with `ordered_pass_infos`, and keep long-lived GPU resources in non-send resources prepared before queueing.
- **Runtime Diagnostics**: Use `Diagnostics::record` for lightweight scalar metrics that need to appear in tooling, overlays, or logs. Prefer this resource for frame/runtime counters before adding a new profiling dependency. Scene renderer counters use the exported `SCENE_*` labels.
...
- **Asset Events**: Built-in render asset stores publish retained `Assets<T>` change records into `Events<AssetChange<T>>` through `publish_asset_change_events`. Use `AssetStore<T>` for custom engine-facing stores that should expose the same ECS event path without draining their change logs.
...
- **Async glTF Spawn Flow**: Queue loads via `request_gltf_scene_spawn(...)`; `gltf_scene_spawn_system` resolves handles, records external buffer/image URIs as glTF scene dependencies, publishes imported meshes into `MeshCache`, images into `TextureImageAssets`, materials into `MaterialDescriptorAssets`, transform animation clips into `TransformAnimationClipAssets`, and skeletal skins into `SkeletonSkinAssets` as labeled assets when source paths are known, registers imported material descriptors in `SceneMaterialLibrary`, attaches `MeshFilter`/`MaterialFilter`/`SkinFilter` plus computed `RenderBounds` and material/tint/texture `RenderMesh` intent, tags spawned entities with `GltfSceneInstance`, `GltfNodeRef`, `GltfSkinRef`, and `TransformAnimationTarget`, and spawns hierarchy roots retrievable through `take_spawned_scene_roots(...)`. Imported material descriptors preserve glTF PBR factors, alpha mode, base-color texture refs, normal texture refs, and generated metallic/roughness refs from packed metallic-roughness maps. Imported mesh assets keep CPU-side vertices, indices, and optional `MeshSkinning` joint/weight attributes for skinning upload paths. The automatic scene renderer draws `MeshFilter` entities from `MeshCache`, resolves `MaterialFilter` through `MaterialDescriptorAssets` before copied `RenderMesh` material data, uses `RenderBounds` for per-camera culling, and binds labeled `TextureImageAssets` for material refs such as `#image_0`; `reload_gltf_scene_path(...)` and `reload_changed_gltf_scenes(...)` replace existing imported entities for the same handle instead of duplicating stale hierarchies.
- **Game Authoring Path**: Prefer `.oxscene`, `SceneDescriptor`, `ScenePrefabDescriptor`, `ScenePrefabOverride`, `SceneSpriteDescriptor`, `SpawnedSceneInstance`, `spawn_scene_descriptor_instance`, `try_spawn_scene_descriptor_instance`, `spawn_scene_prefab_instance`, `try_spawn_scene_prefab_instance`, `scene_descriptor_from_world`, `scene_descriptor_from_roots`, `scene_prefab_from_roots`, `SceneExportError`, `SceneInstanceId`, `scene_instance_id`, `scene_instance_index`, `SceneInstanceRegistry`, `despawn_scene_instance`, `entities_in_scene_instance`, `SceneEntityPath`, `scene_entity_path`, `entity_by_scene_path_in_instance`, `entities_under_scene_path_in_instance`, `Tags`, `entities_with_tag_in_instance`, `first_entity_with_tag_in_instance`, `entity_has_tag`, `SceneDescriptor::validate`, `request_oxscene_spawn`, `reload_changed_native_assets`, `poll_native_asset_reloads`, `NativeAssetReloadSummary`, `reload_changed_render_assets`, `poll_render_asset_reloads`, `RenderAssetReloadSummary`, `reload_oxscene_path`, `reload_changed_oxscenes`, `request_material_descriptor_load`, `reload_changed_material_descriptors`, `MaterialDescriptorAssets`, `TextureImageAssets`, `MaterialFilter`, `request_gltf_scene_spawn`, `reload_gltf_scene_path`, `reload_changed_gltf_scenes`, `GltfSceneInstance`, `GltfNodeRef`, `GltfSkinRef`, `TransformAnimationClipAssets`, `SkeletonSkinAssets`, `AnimationPlayer`, `AnimationBlendPlayer`, `AnimationBlendLayer`, `AnimationState`, `AnimationStateMachine`, `SkinFilter`, `SkinJointMatrices`, `CpuSkinnedMesh`, `SceneMaterialLibrary`, `CameraRenderView`, `CameraViewport`, `viewport_pick_ray`, `pick_scene`, `pick_scene_from_viewport`, `PickingPlugin`, `PickingState`, `PickEvent`, `RenderMesh`, `RenderLayers`, `RenderBounds`, `RenderCullDistance`, `SpriteImage`, `SpriteAssets`, `SpriteBillboard`, `Visibility`, `InheritedVisibility`, `TransformTween`, `GameUi::sprite`, `Terrain`, `TerrainDescriptor`, `SceneWorldDescriptor`, `SceneAuthoringPlugins`, `SceneRendererPlugin`, `GameUiPlugin`, `GameFonts`, `GameTextStyle`, `AudioPlugin`, `Audio`, `SceneEditorPlugin`, `Events<T>`, `Timer`, `RuntimeUiPlugin`, and `DevOverlayPlugin` for new examples and game-facing features. See `docs/building_games.md` and `docs/scene_authoring.md`.
...
- **Contextual Input Flow**: For menus, gameplay, editor tools, and overlays, insert `InputContexts<Mode>`, `ContextualActionBindings<Action, Mode>`, and/or `ContextualAxisBindings<Axis, Mode>`, then use the contextual sync systems so active mode layers drive the same `ActionInput` / `AxisInput` resources.
...
- **Material Descriptors**: Materials can be defined using legacy JSON/RON/TOML descriptors, versioned `.oxmat` wrappers loaded via `load_material_descriptor`, or top-level `.oxscene` `materials`; descriptors support `alpha_mode` values `opaque`, `mask`, and `blend` plus `base_color`, `metallic_factor`, `roughness_factor`, `emissive_color`, and albedo/normal/metallic/roughness texture refs; descriptors loaded through `DefaultPlugins` are registered into `SceneMaterialLibrary` by material name/base color/material factors/alpha mode/material textures and publish authored texture paths into `TextureImageAssets`, while `.oxscene` mesh materials can reference them with `"ref"`. The automatic scene renderer binds albedo, normal, metallic, and roughness texture slots with a neutral normal map and white fallbacks when a slot is absent.
- **Post-Process Passes**: Prefer `RenderTexture`, `RenderTextureDescriptor`, `SceneRenderTarget`, `prepare_scene_renderer_for_target`, `queue_scene_renderer_to_target`, and `FullscreenBlitPipeline` for offscreen color targets, capture/minimap/editor viewport textures, and fullscreen composite passes instead of hand-rolling texture/sampler/pipeline setup in examples.
