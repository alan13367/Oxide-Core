...
- **`oxide_asset`**: Generic typed handles, revision-tracked handle-indexed asset storage with added/modified/removed change records and per-consumer change cursors, async loading/reload primitives, ready imported asset registration, load status, typed path identity, labeled sub-asset identity, and dependency path metadata for hot reload/import invalidation. Renderer-specific caches and imported asset types should live in renderer or engine-facing crates.
...
- **Async glTF Spawn Flow**: Queue loads via `request_gltf_scene_spawn(...)`; `gltf_scene_spawn_system` resolves handles, publishes imported meshes into `MeshCache` and imported materials into `MaterialDescriptorAssets` as labeled assets when source paths are known, registers imported material descriptors in `SceneMaterialLibrary`, and spawns hierarchy roots retrievable through `take_spawned_scene_roots(...)`.
