...
- **`oxide_renderer`**: A low-level abstraction over `wgpu`. It handles device/queue initialization, swapchain management (Surface), and provides primitives for meshes, CPU-side texture images, GPU textures, pipelines, descriptor-driven materials, and shaders.
