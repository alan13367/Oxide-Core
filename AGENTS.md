...
- **`oxide_renderer`**: A low-level abstraction over `wgpu`. It handles device/queue initialization, swapchain management (Surface), and provides primitives for meshes, CPU-side texture images, GPU textures, pipelines, descriptor-driven materials, shaders, and lightweight glTF import data including transform animation channels, skin bind data, and primitive joint/weight attributes.
