...
- **Async glTF Spawn Flow**: Queue loads via `request_gltf_scene_spawn(...)`; `gltf_scene_spawn_system` resolves handles, publishes imported meshes into `MeshCache`, images into `TextureImageAssets`, and materials into `MaterialDescriptorAssets` as labeled assets when source paths are known, registers imported material descriptors in `SceneMaterialLibrary`, attaches `MeshFilter`/`MaterialFilter` plus material/tint `RenderMesh` intent, and spawns hierarchy roots retrievable through `take_spawned_scene_roots(...)`. The automatic scene renderer draws `MeshFilter` entities from `MeshCache`.
