...
- **Input Mapping**: `oxide_input` supports raw keyboard/mouse resources plus semantic `ActionBindings<T>`/`ActionInput<T>` synced by `sync_action_input_system::<T>`; bindings can be replaced, unbound, and inspected for conflicts for settings/editor workflows.
