...
- Load descriptor-driven materials from files via `load_material_descriptor(...)` (supports legacy JSON/RON/TOML plus versioned `.oxmat`, including material `base_color`, `metallic_factor`, `roughness_factor`, `emissive_color`, `alpha_mode` values `opaque`/`mask`/`blend`, and albedo texture references)
