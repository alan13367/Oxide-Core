...
- **Scene Material Library**: reusable named scene materials can be registered directly, declared inside `.oxscene` with base colors, alpha modes, and albedo/normal/roughness texture labels, populated from loaded `.oxmat` descriptors, and referenced from `.oxscene` meshes
...
`conflicting_triggers`. Use `InputContexts<T>` with
`ContextualActionBindings` / `ContextualAxisBindings` when controls should
change by mode, such as gameplay, menu, editor, or paused overlays.
...
materials preserve glTF PBR factors, alpha mode, base-color textures, normal
textures, and generated roughness textures from packed metallic-roughness maps
as Oxide material descriptors. Spawned
...
- Load descriptor-driven materials from files via `load_material_descriptor(...)` (supports legacy JSON/RON/TOML plus versioned `.oxmat`, including material `base_color`, `metallic_factor`, `roughness_factor`, `emissive_color`, `alpha_mode` values `opaque`/`mask`/`blend`, and albedo/normal/roughness texture references)
- Load material descriptors asynchronously with `request_material_descriptor_load(...)`; `DefaultPlugins` publishes ready descriptors into `MaterialDescriptorAssets`, registers them in `SceneMaterialLibrary` by descriptor name/base color, tracks shader/texture dependencies for in-place reloads, publishes authored material texture files into `TextureImageAssets`, and lets `MaterialFilter` render directly from descriptor handles
- The automatic scene renderer binds albedo, normal, and roughness material texture slots together, using a neutral normal map and white fallbacks for missing slots
