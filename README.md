...
- **Post-Process Foundations**: renderer-owned sampled `RenderTexture` targets, scene-render-to-target helpers, and `FullscreenBlitPipeline` utilities for capture, editor viewport, minimap, and future post-processing passes
...
textures, and generated metallic/roughness textures from packed
...
Imported glTF transform animation channels are converted into
`TransformAnimationClip` assets labeled by animation index. Spawned glTF nodes
...
- Load descriptor-driven materials from files via `load_material_descriptor(...)` (supports legacy JSON/RON/TOML plus versioned `.oxmat`, including material `base_color`, `metallic_factor`, `roughness_factor`, `emissive_color`, `alpha_mode` values `opaque`/`mask`/`blend`, and albedo/normal/metallic/roughness texture references)
...
- The automatic scene renderer binds albedo, normal, metallic, and roughness material texture slots together, using a neutral normal map and white fallbacks for missing slots
